...
class XpiredDatabase:
    ...
    def create_tables(self) -> None:
        ...
        try:
            ...
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_expiration_date ON expiration_dates(expiration_date)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_food_item_name ON food_items(name)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_notification_type ON notifications(notification_type)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_status ON expiration_dates(status)')
    ...
    def add_food_item(self, name: str, brand: str = None, category: str = None, 
                     barcode: str = None, image_path: str = None, notes: str = None) -> int:
        """
        ...
        try:
            ...
            logger.info(f"Added food item: {name} (ID: {food_item_id})")
            return food_item_id
    ...
    def add_expiration_date(self, food_item_id: int, expiration_date: str, 
        ...
        try:
            ...
            logger.info(f"Added expiration date for food item {food_item_id}: {expiration_date}")
            return expiration_id
            
        except sqlite3.Error as e:
            logger.error(f"Error adding expiration date: {e}")
            self.connection.rollback()
            raise
    ...
    def add_notification(self, food_item_id: int, expiration_id: int, 
        ...
        try:
            ...
            logger.info(f"Added notification for food item {food_item_id}")
    ...
    def update_item_status(self, expiration_id: int, status: str) -> None:
        ...
        try:
            ...
            self.connection.commit()
            logger.info(f"Updated expiration record {expiration_id} status to {status}")
    ...
    def get_all_food_items(self) -> List[Dict]:
        ...
        try:
            ...
            logger.error(f"Error getting all food items: {e}")
            raise
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Add some food items
        milk_id = db.add_food_item("Milk", "Organic Valley", "Dairy", notes="2% milk")
        bread_id = db.add_food_item("Whole Wheat Bread", "Dave's Killer Bread", "Bakery")
        eggs_id = db.add_food_item("Eggs", "Happy Eggs", "Dairy", notes="Free range")
        ...
        for item in expiring_items:
            print(f"- {item['name']} ({item['brand']}) expires on {item['expiration_date']} - {item['urgency_level']}")
        ...
        for item in all_items:
            print(f"- {item['name']} (Active: {item['active_quantity']}, Earliest expiration: {item['earliest_expiration']})")
