...
class YOLO11Trainer:
    """Train and validate a YOLO11 model using a YAML configuration."""
    ...
    def get_device(self) -> str:
        ...
        if torch.cuda.is_available():
            ...
            if estimated_memory > gpu_memory * 0.8:  # Leave 20% margin
                print(
                    f"Warning: Estimated memory usage ({estimated_memory:.1f} GB) "
                    "may exceed GPU memory"
                )
                print(
                    "Consider reducing batch_size if you encounter CUDA out of memory errors"
                )
    ...
    def validate(self, model_path: str = None):
        ...
        if model_path is None:
            ...
            model_path = (
                f"{self.config.get('project', 'runs/train')}/"
                f"{self.config.get('name', 'exp')}/weights/best.pt"
            )
...
def main():
    """CLI entry point for training/validating YOLO11."""
    parser = argparse.ArgumentParser(
        description="YOLO11 Training for Expiration Date Detection"
    )
